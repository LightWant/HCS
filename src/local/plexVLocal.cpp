...
printf("cnt:%u\n", cnt);
for(ui e = 0; e < g.n; e++) printf("e_cnt %u, %u %.0f\n", e, ec[e], answers[e]);
#endif
}
...
#endif
// if(p>0)
        for(auto u : Pn) answers[u] += CN[p-1][q-h-1];
//         for(ui i = 0; i < p; i++) {
//             answers[Pn[i]] += CN[p-1][q-h-1];
// #ifdef DDDEBUG
// printf("ans, %u, +%.0f pp\n", u, CN[p-1][q-h-1]);
// #endif
//         }
        for(auto u : H) answers[u] += CN[p][q-h];
//         for(ui i = 0; i < h; i++) {
// #ifdef DDDEBUG
// printf("ans, %u, +%.0f ph\n", i, CN[p][q-h-1]);
// #endif
//             answers[H[i]] += CN[p][q-h];
//         }
    };

    if(h == q-1) {
        // for(ui i = 0; i < h; i++) answers[H[i]] += p + edC;
        // for(ui i = 0; i < p; i++) answers[Pn[i]] += 1;
        for(ui u : H) answers[u] += p + edC;
        for(ui u : Pn) answers[u] += 1;
        for(ui i = 0; i < edC; i++) answers[C[i]] += 1;
        return;
    }
...
Pn.pop_back();p--;
    ...
    if(Pn.size() + H.size() + edC < q) return;
